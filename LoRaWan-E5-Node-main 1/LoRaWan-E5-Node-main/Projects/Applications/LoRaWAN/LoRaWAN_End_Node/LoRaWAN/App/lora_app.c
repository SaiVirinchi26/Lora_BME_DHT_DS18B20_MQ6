...
{
	  uint16_t temperature1 = 0,humidity1 = 0,pressure1 =0;
...
void BME_280_Init(void)
{
   ...
   // APP_LOG(TS_OFF, VLEVEL_L, "@@@@@@@ return val  : ~%d\r\n",ret );
}
...
static void SendTxData(void)
{
  ...
  uint16_t i = 0,mode = 1,battery = 3000;

  UTIL_TIMER_Time_t nextTxIn = 0;
#ifdef __BME280 SENSOR__

  /*#######################__BME280 SENSOR__#################*/

  	 uint16_t Tx_temp=0, Tx_press=0, Tx_hum=0;
...
#ifdef __DS1820 SENSOR__
  	 /*#######################__DS1820 SENSOR__##################*/

      uint16_t ds18b20_temp =  DS18B20_Sensor_read();
#endif

#ifdef __DHT11 SENSOR__
      ...
      /*#######################__DHT11 SENSOR__##############*/
            ...
            gcvt(Humidity,7,Hmsg);
            APP_LOG(TS_ON, VLEVEL_M, "DHT11 Humidity : %s\r\n",Hmsg);
            gcvt(Temperature,7,Tmsg);
...
//    AppData.Buffer[i++] = (uint8_t)((ds18b20_temp >> 8) & 0xFF);
//    AppData.Buffer[i++] = (uint8_t)(ds18b20_temp & 0xFF);
//    AppData.Buffer[i++] = (uint8_t)((humidity1 >> 8) & 0xFF);
//    AppData.Buffer[i++] = (uint8_t)(humidity1& 0xFF);
//    AppData.Buffer[i++] = (uint8_t)((pressure1 >> 8) & 0xFF);
//    AppDat.Buffer[i++] = (uint8_t)(pressure1 & 0xFF);
